...
# --- SECURITY ENHANCED APPLICATION STYLE LAYER (FORCED ARIAL 8PX FOR MAXIMUM DENSITY) ---
st.markdown("""
<style>
...
/* GLOBAL OVERSHOT FORCE TO ARIAL 8PX FOR INSTANT CLARITY AT A GLANCE */
html, body, [data-testid="stAppViewContainer"], [class*="st-"], .stMarkdown, p, div, span, label, table, th, td {
...
/* HIGH INTENSITY DENSE CSS MARITIME ANIMATION EFFECT */
@keyframes grid-glow {
    0% { border-color: #1e293b; box-shadow: 0 0 2px rgba(56,189,248,0.1); }
    50% { border-color: #3b82f6; box-shadow: 0 0 6px rgba(56,189,248,0.3); }
    100% { border-color: #1e293b; box-shadow: 0 0 2px rgba(56,189,248,0.1); }
}
.maritime-grid-box {
    border: 1px solid #1e293b;
...
# --- APPLICATION HEADER ---
st.markdown("""
<div style="background-color: #0b0f19; padding: 10px; border-radius: 4px; border: 1px solid #1f2937; margin-bottom: 10px;">
    <h5 style="color: #3b82f6; margin: 0; font-family: 'JetBrains Mono', monospace; font-size: 8px; letter-spacing: 0.5px;">SYSTEM MATRIX // INSTITUTIONAL MACRO TRANSMISSION CORE</h5>
    <h2 style="margin: 2px 0; color: #ffffff; font-size: 11px !important;">🇮🇳 India Energy Shock & Margin Stress Engine</h2>
    <p style="color: #9ca3af; margin: 0; font-size: 8px;">Simulating input cost propagation vectors, retail food shocks, and listed equity margin compression maps across sub-continental trade networks.</p>
</div>
""", unsafe_allow_html=True)

# --- SIDEBAR INTERFACE (CONTROL DECK) ---
with st.sidebar:
    st.markdown("<h4 style='color: #ffffff; margin-bottom: 6px;'>Simulation Control Deck</h4>", unsafe_allow_html=True)
    st.markdown("<p style='color:#9ca3af; font-size:8px; margin-bottom:2px;'>MANUAL BENCHMARK CONFIGURATION</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<p style='color:#9ca3af; font-size:8px; margin-bottom:2px;'>SUPPLY CHAIN DISRUPTION CONTROLS</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<p style='color:#9ca3af; font-size:8px; margin-bottom:2px;'>MACRO TRANSMISSION COEFFICIENTS</p>", unsafe_allow_html=True)
...
with m6:
    st.markdown(f"""
    <div style='text-align: center; background-color: #0b0f19; border: 1px solid #1f2937; border-radius: 2px; padding: 2px;'>
        <p style='color: #9ca3af; margin: 0; font-size: 8px; font-weight: 500;'>System Risk Matrix State</p>
        <p style='color: {state_color}; margin: 0; font-size: 8px; font-weight: bold;'>{system_state}</p>
        <div style='width: 6px; height: 6px; background-color: {state_color}; border-radius: 50%; display: inline-block;'></div>
    </div>
    """, unsafe_allow_html=True)
...
# --- TAB 1: FOOD TECH SYSTEM ---
with t1:
    st.markdown("<h3>🍱 Food Delivery Platform Operating Margin Matrix</h3>", unsafe_allow_html=True)
    base_last_mile = 28.5
    ...
    col1.metric("Simulated Last-Mile Cost", f"₹{simulated_last_mile:.2f} / order", f"{((simulated_last_mile-base_last_mile)/base_last_mile):+.1%} vs Base")
    col2.metric("Customer Order Leakage", f"-{customer_leakage:.2f}%", "Order Elasticity Churn")
    margin_loss = ((simulated_last_mile - base_last_mile) / base_last_mile) * 4.2
    projected_margin = 5.8 - margin_loss
    col3.metric("Projected Platform Margin", f"{projected_margin:.2f}%", f"{-margin_loss:+.2f}% Compression", delta_color="inverse")
    ...
    scaled_costs = [18.0 * (1 + (mandi_disruption - 1) * 0.2), 6.5 * (diesel_price / 92.5), 1.5, 1.5, 1.0]
    ...
    fig_food.update_layout(barmode='group', template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=180, margin=dict(l=10, r=10, t=15, b=10), font=chart_font)
    st.plotly_chart(fig_food, use_container_width=True)

# --- TAB 2: KITCHEN THALI LOGISTICS ---
with t2:
    st.markdown("<h3>🌾 Agricultural Supply Chain Shock & Inter-State Bottlenecks</h3>", unsafe_allow_html=True)
    ...
    if show_mandi:
        ...
        with c1:
            fig_crops = px.bar(df_crops, x='Projected Cost Shift (%)', y='Commodity', orientation='h', color='Projected Cost Shift (%)', color_continuous_scale='Oranges', template='plotly_dark')
            fig_crops.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=200, margin=dict(l=10, r=10, t=10, b=10), font=chart_font)
            st.plotly_chart(fig_crops, use_container_width=True)
        with c2:
            st.markdown(f"""
            <div style="background-color: #0b0f19; padding: 8px; border-radius: 2px; border-left: 2px solid #ea580c; border: 1px solid #1f2937; margin-top: 15px;">
                <h4 style="margin-top: 0;">📋 LOGISTICS PROFILE: {selected_crop.upper()}</h4>
                <p style="color: #9ca3af; font-size: 8px;"><b>Primary Logistics Ports/Hubs:</b> {crop_profiles[selected_crop]['ports']}</p>
                <hr style="border-color: #1f2937; margin: 4px 0;">
                <h3 style="color: #ea580c; margin: 0; font-size: 9px !important;">Current Simulated Pipeline Inflation: {current_crop_shock:.2f}%</h3>
            </div>
            """, unsafe_allow_html=True)

# --- TAB 3: FMCG DEFENSE DOSSIERS ---
with t3:
    st.markdown("<h3>🏭 FMCG Listed Equity Gross Margin Sensitivity Analysis</h3>", unsafe_allow_html=True)
    ...
    f2.metric("HDPE Rigid Packaging Premium", f"{hdpe_inflation:+.2f}%", "Plastic Containers")
    ...
    df_fmcg = pd.DataFrame({'Corporate Entity': companies, 'Historical Gross Margin (%)': gross_margin_baselines, 'Simulated Target Margin (%)': simulated_margins})
    ...
    fig_fmcg.update_layout(barmode='group', template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=180, margin=dict(l=10, r=10, t=10, b=10), font=chart_font)
    st.plotly_chart(fig_fmcg, use_container_width=True)

# --- TAB 4:🚢 MARITIME SOURCING MAPS (INSTITUTIONAL ENHANCED VECTOR RUNTIME) ---
with t4:
    st.markdown("<div class='maritime-grid-box'><h3>🚢 Institutional Ocean Freight Transmission & Coastal Terminal Metrics</h3>"
                "<p style='color:#9ca3af; margin:0;'>Dynamic mapping of landing premiums across primary western coastal gate infrastructure (Nhava Sheva/JNPT, Mundra) tracking Red Sea/Capesize transit re-routing stresses.</p></div>", unsafe_allow_html=True)
    ...
    # Calculate factual micro-indicators linked directly to user inputs
    simulated_route_delay = 10.5 + (freight_shock * 0.05)
    vlsfo_bunker_cost = 580 + (brent_crude * 2.15)
    blank_sailings_rate = min(45.0, 12.5 + (freight_shock * 0.12))
    port_dwell_hours = 22.4 + (mandi_disruption * 14.5)
    
    st.markdown("<div style='margin-top: 4px;'></div>", unsafe_allow_html=True)
    sc1, sc2, sc3, sc4, sc5 = st.columns(5)
    sc1.metric("Import Container Rate", f"${current_container_cost:.2f}", f"+{freight_shock}% Premium")
    sc2.metric("Suez/Capesize Route Delay", f"+{simulated_route_delay:.1f} Days", "Transit Vector Elongation")
    sc3.metric("VLSFO Bunker Benchmark", f"${vlsfo_bunker_cost:.2f}/MT", "Fuel Surcharge Variable")
    sc4.metric("Blank Sailings Index", f"{blank_sailings_rate:.1f}%", "Capacity Withdrawal Rate")
    ...
    sc5.metric("Inbound Port Risk Tier", urgency_index)

    st.markdown("<hr style='margin: 4px 0; border-color: #1e293b;'>", unsafe_allow_html=True)
    
    # High-density layout splitting factual breakdowns and downstream industry compressions
    sub_col1, sub_col2 = st.columns([4, 6])
    
    with sub_col1:
        labels = ['War Risk Protection Premium', 'Bunker Adjustment Factor (BAF)', 'Currency Adjustment Factor (CAF)', 'Inland Port Depot Surcharges']
        values = [450 * (freight_shock/45.0), 350 * (brent_crude/80.0), 120 * (1 + (calc_wpi*10)), 200 * (diesel_price/92.5)]
        fig_pie = px.pie(names=labels, values=values, template='plotly_dark', color_discrete_sequence=px.colors.sequential.YlOrRd_r)
        fig_pie.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=180, margin=dict(l=5, r=5, t=10, b=5), font=chart_font, showlegend=False)
        st.plotly_chart(fig_pie, use_container_width=True)
        st.markdown("<p style='color:#64748b; text-align:center; margin:0;'>Inbound Component Breakdown Matrix</p>", unsafe_allow_html=True)

    with sub_col2:
        # Factual Impact Matrix Table for Key Listed Commodity Segments
        st.markdown("<b style='color:#38bdf8;'>Downstream Sub-Continental Port Landed Cost Transmission Analysis:</b>", unsafe_allow_html=True)
        
        # Calculate localized landing cost parameters
        chem_impact = (freight_shock * 0.22) + (crude_delta_pct * 0.45)
        pharma_impact = (freight_shock * 0.35) + (crude_delta_pct * 0.15)
        elec_impact = (freight_shock * 0.41)
        steel_impact = (diesel_price - 92.5) * 0.35 + (freight_shock * 0.08)
        
        st.markdown(f"""
        <table style='width:100%; border-collapse: collapse; margin-top: 4px;'>
            <tr style='background-color:#0f172a; text-align:left; border-bottom:1px solid #1e293b;'>
                <th style='padding:3px;'>Import Cargo Category</th>
                <th style='padding:3px;'>Key Inbound Hub</th>
                <th style='padding:3px;'>Operational Transmission Vector</th>
                <th style='padding:3px; text-align:right;'>Simulated Cost Delta</th>
            </tr>
            <tr style='border-bottom:1px solid #0f172a;'>
                <td style='padding:3px;'><b>Specialty Chemicals & Polymers</b></td>
                <td style='padding:3px;'>Mundra / Dahej</td>
                <td style='padding:3px;'>Petrochemical feedstock import baseline pricing spikes via container constraints.</td>
                <td style='padding:3px; text-align:right; color:#ef4444;'>+{chem_impact:+.1f}%</td>
            </tr>
            <tr style='border-bottom:1px solid #0f172a;'>
                <td style='padding:3px;'><b>Pharma Active Ingredients (APIs)</b></td>
                <td style='padding:3px;'>Nhava Sheva (JNPT)</td>
                <td style='padding:3px;'>Air freight substitution due to blank sailings compounding input costs.</td>
                <td style='padding:3px; text-align:right; color:#ef4444;'>+{pharma_impact:+.1f}%</td>
            </tr>
            <tr style='border-bottom:1px solid #0f172a;'>
                <td style='padding:3px;'><b>Electronic Components & Solar Cells</b></td>
                <td style='padding:3px;'>Mundra Terminal</td>
                <td style='padding:3px;'>Demurrage fees and container detention penalties from clearing backlogs.</td>
                <td style='padding:3px; text-align:right; color:#f59e0b;'>+{elec_impact:+.1f}%</td>
            </tr>
            <tr style='border-bottom:1px solid #1e293b;'>
                <td style='padding:3px;'><b>Metallurgical Scrap & Coking Coal</b></td>
                <td style='padding:3px;'>Kandla Bulk Hub</td>
                <td style='padding:3px;'>Bulk carrier daily charter rates increasing alongside VLSFO indexing.</td>
                <td style='padding:3px; text-align:right; color:#22c55e;'>+{steel_impact:+.1f}%</td>
            </tr>
        </table>
        <div style='margin-top:4px; padding:3px; background-color:#020617; border:1px dashed #334155;'>
            <span style='color:#9ca3af;'><b>Terminal Turnaround Diagnostics:</b> Nhava Sheva container yard dwell times are tracking at <b>{port_dwell_hours:.1f} hours</b>. Surcharge thresholds are triggered automatically when global container availability indexes drop past historic safety medians.</span>
        </div>
        """, unsafe_allow_html=True)

# --- TAB 5: NSE CAPITAL REALIZATION ---
with t5:
    st.markdown("<h3>📊 Nifty Listed Industry Valuation Translation Maps</h3>", unsafe_allow_html=True)
    ...
    df_nse = pd.DataFrame({'NSE Sub-Sector Index': sectors, 'Historical Base Multiple (P/E)': multiples_baseline, 'Projected Target Multiple Shift': multiples_shifts})
    
    fig_nse = px.scatter(df_nse, x='NSE Sub-Sector Index', y='Projected Target Multiple Shift', size=df_nse['Historical Base Multiple (P/E)'], color='Projected Target Multiple Shift', color_continuous_scale='RdYlGn', template='plotly_dark')
    fig_nse.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=180, margin=dict(l=10, r=10, t=10, b=10), font=chart_font)
    st.plotly_chart(fig_nse, use_container_width=True)

# --- TAB 6: MONETARY INTERVENTION STANCE ---
with t6:
    st.markdown("<h3>🏛️ Reserve Bank Stance & Sovereign Yield Trajectories</h3>", unsafe_allow_html=True)
    base_repo = 6.50
...
with t7:
    st.markdown("<h3>📜 Underlying Transmission Matrices & Formula Arrays</h3>", unsafe_allow_html=True)
    st.markdown(r"""
    ...
    """)
    st.markdown("<p style='color:#7c3aed; font-family: 'JetBrains Mono', monospace; font-size:8px;'>VERIFICATION MATRIX SECURITIES SYSTEM ENCRYPTED // END OF PIPELINE</p>", unsafe_allow_html=True)

# --- FOOTER ANCHOR ---
st.markdown("""
<hr style="border-color: #1f2937; margin: 8px 0;">
<div style="text-align: center; color: #6b7280; font-size: 8px; font-family: 'JetBrains Mono', monospace; padding-bottom: 10px;">
